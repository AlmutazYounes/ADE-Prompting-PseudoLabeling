...
DEFAULT_EPOCHS = 10  # Increased epochs with early stopping
DEFAULT_LEARNING_RATE = 3e-5  # Slightly lower learning rate for stability
...
TRAINING_ARGS = {
    ...
    "max_grad_norm": DEFAULT_GRADIENT_CLIP,
...
def process_raw_ner_data(data: List[Dict], tokenizer) -> List[Dict]:
    """Process raw text and entity spans into tokens and tags with improved alignment"""
    processed_data = []
    for item in tqdm(data, desc="Processing data", ncols=100):
        ...
        # Assign BIO tags based on entity spans with improved boundary detection
        for entity in entities:
            ...
            # Apply BIO tagging to entity tokens
...
def tokenize_and_align_labels(examples, tokenizer, label2id, max_length=DEFAULT_MAX_LENGTH):
    """Tokenize inputs and align labels for model training with improved subword handling"""
    ...
    for i, tags in enumerate(examples["tags"]):
        ...
        for word_idx in word_ids:
            if word_idx is None:
                ...
                # For subwords, copy the label from the first token
                # This is different from the original approach and helps maintain entity integrity
        ...
        # Ensure first token is not -100
        if label_ids and label_ids[0] == -100:
            label_ids[0] = label2id["O"]
...
def create_compute_metrics_fn(id2label):
    ...
    def compute_metrics(eval_pred):
        ...
        # Calculate metrics for individual entity types
        entity_metrics = {}
        for key in results:
            if key not in ['overall_precision', 'overall_recall', 'overall_f1']:
                entity_metrics[key] = results[key]
...
class BertCRFForNER(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, labels=None):
        ...
        if labels is not None:
            # Create a mask from labels to mask out -100 values
            mask = (labels >= 0)
            # Replace -100 with a valid label (0) for CRF computation
            labels_fixed = labels.clone()
            labels_fixed[labels < 0] = 0
...
class EnhancedTrainer(Trainer):
    ...
    def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval"):
        ...
        if "eval_overall_f1" in metrics:
            ...
            # Log entity-level metrics if available
            if 'eval_entity_metrics' in metrics:
                logger.info("-" * 50)
                logger.info("Entity-level metrics:")
                for entity, entity_metrics in metrics['eval_entity_metrics'].items():
                    if isinstance(entity_metrics, dict) and 'f1' in entity_metrics:
...
def train_model(
    ...
    warmup_ratio = config.get("warmup_ratio", DEFAULT_WARMUP_RATIO)
    weight_decay = config.get("weight_decay", DEFAULT_WEIGHT_DECAY)
    gradient_clip = config.get("gradient_clip", DEFAULT_GRADIENT_CLIP)
    ...
    if needs_processing:
        ...
        processed_data = process_raw_ner_data(processed_data, tokenizer)
    
    # Prepare labels and datasets
    label2id, id2label, labels = prepare_labels(processed_data)
    ...
    logger.info(f"Training steps: {num_train_steps}, Warmup steps: {warmup_steps}")
...
def run_training_pipeline(config: Dict = None):
    ...
    for data_source in data_sources:
        ...
        for model_name in models_to_train:
            ...
            model_output_dir = os.path.join(run_dir, f"{data_source}_{model_name}_enhanced")
